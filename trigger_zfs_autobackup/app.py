...
class UdevAutobackupMonitor:
    ...
    def _device_callback(self, device: pyudev.Device) -> None:
        ...
        if fs_type == "zfs_member" and fs_label in self.config.pools and device.action in ("add", "remove"):
    ...
    def run(self) -> None:
        ...
        observer = pyudev.MonitorObserver(self.udev_monitor, callback=self._device_callback)
        observer.start()
...
def init_logging(run_as_daemon: bool) -> list[IO]:
...
def main() -> None:
    ...
    if args.action != Action.STOP:
        deamonise = args.action in {Action.START, Action.RESTART}
        files_preserve = init_logging(deamonise)

        app = UdevAutobackupMonitor(config)
        files_preserve.append(app.udev_monitor)

        if args.action == Action.TEST:
            app.test()
        else:
            if deamonise:
                daemon_ctx = DaemonContext(pidfile=PidFile(PID_FILE_PATH), files_preserve=files_preserve)
                daemon_ctx.open()
            app.run()
